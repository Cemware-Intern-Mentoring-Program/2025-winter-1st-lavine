...
## Controller의 메소드 내 아무런 어노테이션이 없을 때 스프링은 이 값들을 어떻게 처리할까
| 파라미터 타입                                                     | Spring MVC 기본 처리 방식                                 |
| ----------------------------------------------------------- | --------------------------------------------------- |
| **도메인 객체(POJO, DTO 클래스 등)**                                 | `@ModelAttribute` 로 처리 → 요청 파라미터 바인딩                |
| **기본형(Primitive), Wrapper, String 등 단순 타입**                 | `@RequestParam` 로 처리 → QueryString/Form Field 바인딩   |
| **HttpServletRequest, Response, InputStream 등 웹 API 관련 타입** | 해당 객체 그대로 주입 (`@RequestParam/@ModelAttribute` 적용 X) |
| **Map, Model, ModelMap**                                    | 모델을 위한 객체로 처리 → View로 전달됨                           |
| **@RequestBody 생략 불가인 경우**                                  | JSON 요청 Body는 자동 처리되지 않음 → 반드시 `@RequestBody` 필요    |
...
---
DTO에 Getter가 들어가는 이유는 직렬화/역직렬화 때문