...
### Front Controller 패턴 
모든 클라이언트 요청을 단일 진입점에서 공통 처리한 뒤 필요한 핸들러(Controller)에게 분배하는 구조

공통 로직을 중앙 처리하기 때문에 인증이나 인코딩, 라우팅 에러 로그 관리에 용이하며 유연한 확장이 가능하다.

> 프론트 컨트롤러는 클라이언트 요청을 받아서 view resolver나 handler mapping , adpater 등에 보내주는 역할을 한다. 프론트에서 컨트롤러가 받아서 요청을 해서 보내준다. 프론트 컨트롤러 개념이 된다. 구성 요소로서 인터셉터, AOP등이 존재하는 것 
